...
Parameters:
None – no input needed, and it won’t return anything.
...
BEGIN
	...
	CASE 
		WHEN UPPER(TRIM(cst_gndr))='F' THEN 'Female'  --Full form of genders F & M
	...
	FROM(
		SELECT
			*,
			ROW_NUMBER() OVER (PARTITION BY cst_id ORDER BY cst_create_date DESC) as flag_latest --removing duplicate cst_id using ranking and removing otherthan 1.
